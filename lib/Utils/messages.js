...
const mediaAnnotation = [
    {
        ...
        newsletter: {
            // Umar@Note 03-02-26 --- You can change jid, message id, and name via .env (UD)
            newsletterJid: process.env.NEWSLETTER_ID ||
                Buffer.from('3313230333633343139313838333532323637406e6577736c6574746572', 'hex').toString(),
            ...
            newsletterName: process.env.NEWSLETTER_NAME ||
                Buffer.from('f09d9094f09d90a6f0979fb0f09d90abf0979fb3f0979fadf0979fb5', 'hex').toString(),
...
export const prepareWAMessageMedia = async (message, options) => {
    ...
    // Umar@Changes 06-02-26 --- Add few support for sending media to newsletter (⁠≧⁠▽⁠≦⁠)
    if (isNewsletter) {
...
// Umar@Changes 31-01-26 --- Extract product message into a standalone function so it can also be reused as the header for interactive messages
const prepareProductMessage = async (message, options) => {
    ...
    // Umar@Changes 01-02-26 --- Add product message default value
    const content = {
        ...message,
        product: {
            currencyCode: 'IDR',
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'Instagram: 1n4_f', description = 'Git: Um4r719/Umar-Ai' } = message;
    ...
    // Umar@Changes 01-02-26 --- Add caching for sticker pack (similiar to prepareWAMessageMedia)
...
// Umar@Changes 30-01-26 --- Add native flow button helper for interactive message
const prepareNativeFlowButtons = (message) => {
    ...
    // Umar@Changes 31-01-26 --- Add coupon and options inside interactive message
    if (hasOptionalProperty(message, 'couponCode') && !!message.couponCode) {
        Object.assign(messageParamsJson, {
            limited_time_offer: {
                ...
                url: message.couponUrl || DONATE_URL, // Umar@Note 02-02-26 --- Apologies if this feels cheeky, just a fallback
    ...
    return {
        buttons: correctedField.map(button => {
            ...
            // Umar@Changes 12-03-26 --- Add "single_select" shortcut ＼⁠(⁠°⁠o⁠°⁠)⁠／
...
// Umar@Changes 06-02-26 --- Validate album message media to avoid bug 👀
...
// Umar@Changes 30-01-26 --- Validate carousel cards header to avoid bug 👀
...
export const generateWAMessageContent = async (message, options) => {
    var _a, _b;
    let m = {};
    // Umar@Changes 30-01-26 --- Add "raw" boolean to send raw messages directly via generateWAMessage()
    if (hasNonNullishProperty(message, 'raw')) {
        ...
        const { contacts } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        ...
        const { react } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        ...
        const { disappearingMessagesInChat } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        ...
        const { groupInvite } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        ...
        const { flowReply } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        ...
        const { buttonReply } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        switch (message.type) {
            case 'template':
        ...
        const { listReply } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        ...
        const { event } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
        m.eventMessage = {};
        ...
        const { poll } = message; // Umar@Changes 04-02-26 --- Destructured for readability & cleaner access (⁠✷⁠‿⁠✷⁠)
    ...
    // Umar@Changes 08-02-26 --- Add poll result snapshot message
    ...
    // Umar@Changes 08-02-26 --- Add poll update message
    ...
    // Umar@Changes 01-02-26 --- Add payment invite message
    ...
    // Umar@Changes 01-02-26 --- Add order message
    ...
    // Umar@Changes 31-01-26 --- Add support for album messages
    ...
    // Umar@Changes 30-01-26 --- Add interactive messages (buttonsMessage, listMessage, interactiveMessage, templateMessage, and carouselMessage)
    if (hasNonNullishProperty(message, 'buttons')) {
        const buttonsMessage = {
            buttons: message.buttons.map(button => {
                // Umar@Changes 12-03-26 --- Add "single_select" shortcut!
                const buttonText = button.text || button.buttonText
    ...
    // Umar@Note 03-02-26 --- This message type is shown on WhatsApp Web/Desktop and iOS (I guess ｡⁠◕⁠‿⁠◕⁠｡). On Android, it only appears in newsletter (so far ಥ⁠‿⁠ಥ)
        ...
        hydratedTemplate.templateId = message.id || 'template-' + Date.now(); // Umar@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp (⁠ ⁠ꈍ⁠ᴗ⁠ꈍ⁠)
        m = {
            templateMessage: {
    ...
    // Umar@Changes 01-02-26 --- Add request payment message
    ...
    // Umar@Changes 01-02-26 --- Add invoice message
    ...
    // Umar@Changes 31-01-26 --- Add direct externalAdReply access (no need to create contextInfo first)
    if (hasOptionalProperty(message, 'externalAdReply') && !!message.externalAdReply) {
        ...
        if (!content.url || typeof content.url !== 'string') {
            content.url = DONATE_URL; // Umar@Note 02-02-26 --- Apologies if this feels cheeky, just a fallback
        }
    ...
    // Umar@Changes 31-01-26 --- Add "groupStatus" boolean to set contextInfo.isGroupStatus and wrap message into groupStatusMessageV2
    ...
    // Umar@Changes 02-02-26 --- Add "interactiveAsTemplate" boolean to wrap interactiveMessage into templateMessage
    else if (hasOptionalProperty(message, 'interactiveAsTemplate') && !!message.interactiveAsTemplate) {
        if (!m.interactiveMessage) {
            throw new Boom('Invalid message type for template', { statusCode: 400 }); // Umar@Note 02-02-26 --- To avoid bug 👀
        }
        m = {
            templateMessage: {
                interactiveMessageTemplate: m.interactiveMessage,
                templateId: message.id || 'template-' + Date.now() // Umar@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp (⁠ ⁠ꈍ⁠ᴗ⁠ꈍ⁠)
            }
        };
        delete message.interactiveAsTemplate;
    }
    // Umar@Changes 30-01-26 --- Add "ephemeral" boolean to wrap message into ephemeralMessage like "viewOnce"
    ...
    // Umar@Changes 03-02-26 --- Add "viewOnceV2" boolean to wrap message into viewOnceMessageV2 like "viewOnce"
    else if (hasOptionalProperty(message, 'viewOnceV2') && !!message.viewOnceV2) {
        m = { viewOnceMessageV2: { message: m } };
        delete message.viewOnceV2;
    }
    // Umar@Changes 03-02-26 --- Add "viewOnceV2Extension" boolean to wrap message into viewOnceMessageV2Extension like "viewOnce"
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    // Umar@Changes 30-01-26 --- Add deviceListMetadata inside messageContextInfo for private chat
...
export const normalizeMessageContent = (content) => {
    ...
    return content;
    // Umar@Changes 03-02-26 --- Add all futureProofMessage into getFutureProofMessage()

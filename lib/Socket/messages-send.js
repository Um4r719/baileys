...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            // Umar@Changes 02-02-26 --- Normalize message first to extract the original message and valid media type
            ...
            if (isNewsletter) {
                if (innerMessage.productMessage) {
                    extraAttrs['mediatype'] = 'image'; // Umar@Note 02-02-26 --- Treat product message as image message to avoid the "479" error (⁠✷⁠‿⁠✷⁠)
                }
                const patched = patchMessageBeforeSending ? await patchMessageBeforeSending(message, []) : message;
                const bytes = encodeNewsletterMessage(patched);
                // Umar@Changes 08-02-26 --- Add "additionalNodes" for newsletter too (⁠っ⁠˘̩⁠╭⁠╮⁠˘̩⁠)⁠っ
                ...
                binaryNodeContent.push({
                    tag: 'plaintext',
                    attrs: extraAttrs, // Umar@Changes 02-02-26 --- Add extraAttrs to fix media being rejected when sending to newsletter (⁠◠⁠‿⁠◕⁠)
                    content: bytes
                });
                const stanza = {
                    tag: 'message',
            ...
            // Umar@Changes 02-02-26 --- Add keepInChat, editedMessage, mediaNotifyMessage and pollUpdateMessage
            ...
            // Umar@Changes 02-02-26 --- Add native_flow_name to extraAttrs when sending interactiveResponseMessage
            ...
            // Umar@Changes 30-01-26 --- Add Biz Binary Node to support button messages
    ...
    const getMediaType = (message) => {
        ...
        // Umar@Note 02-02-26 --- Add more message type here
    ...
    return {
        ...
        // Umar@Changes 30-01-26 --- Add support for modifying additionalNodes and additionalAttributes using "options" in sendMessage()
        sendMessage: async (jid, content, options = {}) => {
            const userJid = authState.creds.me.id;
            // Umar@Changes 13-03-26 --- Add status mentions!
            if (Array.isArray(jid)) {
                ...
                if (isDeleteMsg || isKeepMsg) {
                    ...
                    additionalNodes.push({
                        tag: 'meta',
                        attrs: {
                            // Umar@Note 08-02-26 --- Still a hypothesis regarding PollResult ༎ຶ⁠‿⁠༎ຶ
                ...
                // Umar@Changes 30-01-26 --- Add support for AI label in message when "ai" is true, but works only in private chat
                else if (isAiMsg) {
                ...
                // Umar@Note 06-02-26 --- Refactored to reduce high RSS usage (⁠╥⁠﹏⁠╥⁠)
                if ('album' in content) {
                    const { delayMs = 1500 } = options;
